...
export class QualityService {
  ...
  async calculateQuality(meetingId: string): Promise<QualityMetrics> {
    ...
    const confidenceValues = speakers
      .filter(
        (s) => s.nameConfidence !== null && s.nameConfidence !== undefined,
      )
    ...
    const speakerScore = Math.min(
      100,
      speakers.filter((s) => !s.isUnknown).length * 25,
    );
  ...
  private getRecommendations(metrics: QualityMetrics): string[] {
    ...
    if (metrics.inaudibleCount > 5) {
      recommendations.push(
        'Review and correct inaudible sections for better accuracy',
      );
    }
    ...
    if (metrics.avgSegmentLength < 10) {
      recommendations.push(
        'Transcript has short segments - audio quality may be improved',
      );
    }
    if (metrics.speakerCount === 1) {
      recommendations.push(
        'Only one speaker detected - verify if more participants exist',
      );
    }
