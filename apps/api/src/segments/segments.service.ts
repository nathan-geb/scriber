...
export class SegmentsService {
  ...
  async resetToOriginal(segmentId: string, userId: string) {
    const segment = await this.prisma.transcriptSegment.findFirst({
      where: { id: segmentId },
      include: { meeting: { select: { userId: true } } },
    });
    ...
    if (!segment.originalText) {
      throw new NotFoundException(
        'Original text not available for this segment',
      );
    }

    // No change needed if already at original
    if (segment.text === segment.originalText) {
      return segment;
    }

    // Create edit history record and reset text in transaction
    const [, updatedSegment] = await this.prisma.$transaction([
      this.prisma.segmentEdit.create({
        data: {
          segmentId,
          previousText: segment.text,
          newText: segment.originalText,
          editedBy: userId,
          editReason: 'Reset to original transcription',
        },
      }),
      this.prisma.transcriptSegment.update({
        where: { id: segmentId },
        data: { text: segment.originalText },
        include: { speaker: true },
      }),
    ]);
