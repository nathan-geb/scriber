...
const navItems: NavItem[] = [
    ...
    { href: '/meetings', label: 'Meetings', icon: FolderOpen },
            ...
            <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 md:hidden">
                ...
                <div className="flex items-center gap-2 px-3 py-2 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl shadow-black/30">
                    ...
                    <Link
                        ...
                        className={`w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 ${isActive(navItems[0].href)
                                ? 'text-white bg-white/15'
                                : 'text-slate-400 hover:text-white hover:bg-white/5'
                            }`}
                    >
                        <LayoutDashboard size={22} strokeWidth={isActive(navItems[0].href) ? 2.5 : 2} />
                    </Link>

                    {/* Meetings */}
                    <Link
                        href={navItems[1].href}
                        className={`w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 ${isActive(navItems[1].href)
                                ? 'text-white bg-white/15'
                                : 'text-slate-400 hover:text-white hover:bg-white/5'
                            }`}
                    >
                        <FolderOpen size={22} strokeWidth={isActive(navItems[1].href) ? 2.5 : 2} />
                    </Link>
                    ...
                    <Link
                        ...
                        className={`w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 ${isActive(navItems[2].href)
                                ? 'text-white bg-white/15'
                                : 'text-slate-400 hover:text-white hover:bg-white/5'
                            }`}
                    >
                        <User size={22} strokeWidth={isActive(navItems[2].href) ? 2.5 : 2} />
                    </Link>
                </div>
            </nav>
        </>
